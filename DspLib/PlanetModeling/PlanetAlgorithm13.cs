...
public class PlanetAlgorithm13 : PlanetAlgorithm
{
    ...
    public override void GenerateTerrain(double modX, double modY)
    {
        ...
        for (var index = 0; index < data.dataLength; ++index)
        {
            ...
            var x = Remap(0.0, 2.0, 0.0, 4.0,
                Math.Pow(Remap(-1.0, 1.0, 0.0, 1.0, simplexNoise.Noise3DFBM(num4 * num1, num5 * num2, num6 * num3, 6)),
                    modY) * (49.0 / 16.0));
            if (x < 1.0)
                x = Math.Pow(x, 2.0);
            var num7 = Math.Min(x - 0.2, 4.0);
            Math.Max(1.0 - Math.Abs(1.0 - num7), 0.0);
            if (num7 > 2.0)
                num7 = num7 <= 3.0 ? 2.0 - 1.0 * (num7 - 2.0) : num7 <= 3.5 ? 1.0 : 1.0 + 2.0 * (num7 - 3.5);
            var num8 = num7;
            data.heightData[index] = (ushort)((planet.radius + num8 + 0.1) * 100.0);
                ...
                for (var index4 = 0; index4 < num8; ++index4)
                {
                    ...
                    while (num9++ < 200)
                    {
                        ...
                        if (num10 >= (double)planet.radius &&
                            ...
                            (eveinType > EVeinType.Titanium || num10 <= planet.radius + 0.699999988079071))
                        {
                            var flag3 = false;
    ...
    {
        var num1 = 0.007 * planet.mod_x;
        var num2 = 0.007 * planet.mod_x;
        var num3 = 0.007 * planet.mod_x;
        var simplexNoise = new SimplexNoise(new DotNet35Random(planet.seed).Next());
        ...
        for (var index = 0; index < data.dataLength; ++index)
        {
            ...
            var x = Remap(0.0, 2.0, 0.0, 4.0,
                Math.Pow(Remap(-1.0, 1.0, 0.0, 1.0, simplexNoise.Noise3DFBM(num4 * num1, num5 * num2, num6 * num3, 6)),
                    planet.mod_y) * (49.0 / 16.0));
            if (x < 1.0)
                x = Math.Pow(x, 2.0);
            var num7 = Math.Min(x - 0.2, 4.0);
            notClampedBiomo[index] = (float)num7;
        }
    }
}